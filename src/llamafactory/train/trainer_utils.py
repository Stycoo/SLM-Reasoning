...
def get_batch_step_weighted_logps(
    logits: "torch.Tensor", labels: "torch.Tensor", step_weights: "torch.Tensor", 
    step_index: "torch.Tensor", label_pad_token_id: int = IGNORE_INDEX
) -> Tuple["torch.Tensor", "torch.Tensor"]:
    r"""
    Computes the log probabilities of the given labels under the given logits.
    ...
    per_token_logps = torch.gather(logits.log_softmax(-1), dim=2, index=labels.unsqueeze(2)).squeeze(2)
    per_token_logps = per_token_logps * loss_mask * step_weights[:, 1:]
        
    batch_size, max_step_num, _ = step_index.size()
    step_scores = torch.zeros(batch_size, max_step_num, dtype=logits.dtype).to(logits.device)
    for i in range(batch_size):
        for j in range(max_step_num):
            start_idx, end_idx = step_index[i, j]
            
            if start_idx == 0 and end_idx == 0:
                pass
            
            step_logps = per_token_logps[i, start_idx:end_idx+1]
            avg_step_logps = step_logps.mean()
            step_scores[i, j] = avg_step_logps
   
    # step_scores_mask = step_scores != 0
    # step_scores = step_scores.sum(-1) / step_scores_mask.sum(-1)
    step_scores = step_scores.sum(-1)
