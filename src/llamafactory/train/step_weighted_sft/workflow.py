...
from ...data import StepWeightedSFTDataCollator, get_dataset, get_template_and_fix_tokenizer
...
def run_step_weighted_sft(
    ...
    dataset_module = get_dataset(template, model_args, data_args, training_args, stage="step_weighted_sft", **tokenizer_module)
    model = load_model(tokenizer, model_args, finetuning_args, training_args.do_train)

    if getattr(model, "is_quantized", False) and not training_args.do_train:
        setattr(model, "_hf_peft_config_loaded", True)  # hack here: make model compatible with prediction

    data_collator = StepWeightedSFTDataCollator(
