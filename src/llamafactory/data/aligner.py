...
def convert_multi_sft(
    example: Dict[str, Any],
    dataset_attr: "DatasetAttr",
    data_args: "DataArguments",
) -> Dict[str, Any]:
    r"""
    Converts multi sft format dataset to the standard format.
    input: {'prompt':, 'best_response':, 'source_model_outputs':[], 
    'source_rm_scores':[], 'completion_inputs':[], 'completion_outputs':[], 'completion_rm_scores':[]}
    """
    tag_mapping = {
        dataset_attr.user_tag: Role.USER.value,
        dataset_attr.assistant_tag: Role.ASSISTANT.value,
...
def align_dataset(
    ...
    Aligned dataset:
        ...
        _videos: [],
    """
    if dataset_attr.formatting == "alpaca": # "alpaca" "sharegpt" "multi_sft"
