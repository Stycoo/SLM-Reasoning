...
def main(config):
    ...
    # Save metrics to CSV
    csv_path = os.path.join(output_dir, f'{os.path.basename(config.data.path).split(".")[0]}_n_{config.data.n_samples}_temp_{config.rollout.temperature}_topp_{config.rollout.top_p}_maxlen_{config.rollout.response_length}_pass_{spent_hours:.2f}h.csv')
