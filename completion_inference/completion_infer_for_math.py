...
source_model_names = ['deepseek-r1']
...
for data in input_data:
    ...
    for model_name in source_model_names:
        if model_name in data:
            source_model_responses = data[model_name]
            if isinstance(source_model_responses, str):
                source_model_responses = [source_model_responses]

            for i, model_response in enumerate(source_model_responses):
                completion_steps = model_response.split("\n\n")[:-1]
...
for i, output in enumerate(outputs):
    ...
    if prompt in output_data:
        ...
        output_data[prompt] = {'prompt': prompt, 'answer': answer, 'completion_inputs': [completion_input], 'completion_outputs':[generated_text],
