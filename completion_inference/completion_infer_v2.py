...
for data in input_data:
    ...
    _completion_inputs = create_node(best_response, 'sentence', 2, 4)
    if _completion_inputs:
        prompts.append(prompt)
        best_responses.append(best_response)
        best_response_scores.append(best_response_score)
        if len(_completion_inputs) < args.completion_start_index + 1:
            filtered_inputs_size += 1
            continue
        else:
            _completion_input = _completion_inputs[args.completion_start_index]
            # _completion_input = _completion_inputs[-(args.completion_start_index+1)]
        
        completion_inputs.append(f"{prompt_normal}{_completion_input}")

print(f"completion_inputs size: {len(completion_inputs)}, filtered_inputs size: {filtered_inputs_size}")
