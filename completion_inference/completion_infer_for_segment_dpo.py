...
answers = []
completion_inputs = []
gt_next_step_completion_outputs = []
...
for data in input_data:
    ...
    if args.forward_or_reverse == 'reverse':
        ...
        gt_next_step_completion_outputs.append(parts[-args.completion_id])

    # forward
    elif args.forward_or_reverse == 'forward':
        ...
        chosen_rejected_flag.append(0)
        gt_next_step_completion_outputs.append(parts[args.completion_id-1])
...
for i, output in enumerate(outputs):
    ...
    gt_next_step_completion_output = gt_next_step_completion_outputs[i]
    output_data[prompt] = {'prompt': prompt, 'answer': answer, 'completion_inputs': [answer, completion_input], 
                            'completion_outputs':[gt_next_step_completion_output, generated_text], 'chosen_rejected': [1, chosen_rejected]}
