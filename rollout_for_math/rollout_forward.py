...
parser.add_argument("--max_segment_num", type=int, default=0)
...
for data in input_data:
    ...
    if len(completion_steps) <= args.max_segment_num:
        segments = completion_steps[:-1]
    else:
        part_size = len(completion_steps) // (args.max_segment_num)
        remainder = len(completion_steps) % (args.max_segment_num)

        segments = []
        start_index = 0
        for i in range(args.max_segment_num - 1):
