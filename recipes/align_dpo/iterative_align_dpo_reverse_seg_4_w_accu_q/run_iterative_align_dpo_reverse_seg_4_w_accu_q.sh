...
do  
    ...
    if [ "$i" -eq 1 ]; then
        ...
        POST_PROCESS_TYPE=create_aligned_segment_dpo_dataset
        ...
        COMP_FOR_ACCU_Q_INPUT_FILE=$RM_SCORED_OUTPUT_DIR/all_step_rollout_rule_verified.json
        ...
        do
            ...
            GPU=${GPUS[$GPU_INDEX]}
            SCORING_LOG_FILE="${LOG_DIR}/scoring_accu_q_iter_${i}_chunk_${j}.log"
            # 使用 & 符号将任务放到后台运行
            CUDA_VISIBLE_DEVICES=$GPU python $PROJ_DIR/rollout_for_math/rollout_verification.py \
        ...
        POST_PROCESS_TYPE=create_aligned_segment_accu_q_dpo_dataset
        CREATE_ACCU_Q_DPO_LOG_FILE="${LOG_DIR}/create_aligned_dpo_accu_q_iter_${i}.log"
        python $PROJ_DIR/rollout_for_math/build_dataset.py \
            --forward_or_reverse $FORWARD_OR_REVERSE \
            --model_name_or_path "$MODEL" \
            --completion_file_dir $RM_SCORED_OUTPUT_DIR \
            --completion_accu_q_dir $ACCU_Q_RM_SCORED_OUTPUT_DIR \
            --post_process_type $POST_PROCESS_TYPE \
            --iteration_time $ITERATION_TIME \
            --iteration_id $i > "$CREATE_ACCU_Q_DPO_LOG_FILE" 2>&1
