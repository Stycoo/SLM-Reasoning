...
for ((i = 2; i <= ITERATION_TIME; i++)); do
    ...
    # ORM_SCORED_OUTPUT_DIR=/GLOBALFS/gznwp_3/qxj/shitianyuan/Model-Fusion-public/data/FuseChat-3.0-DPO-Data/0520/Llama-3.2-3B-Instruct/iterative_reverse_segment_dpo_w_accu_q/iter_2/rollout_old/completion_with_rm_score/orm_verify
    POST_PROCESS_LOG_FILE="${ROLLOUT_LOG_DIR}/create_align_segment_dpo_dataset_iter_${i}.log"
    python "$PROJ_DIR/rollout_for_math/build_dataset_0520.py" \
        --completion_file_dir "$ORM_SCORED_OUTPUT_DIR" \
        --post_process_type create_align_segment_dpo_dataset \
