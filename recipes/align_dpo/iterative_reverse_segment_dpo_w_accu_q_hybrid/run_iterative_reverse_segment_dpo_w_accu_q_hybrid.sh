...
BASE_MODEL="${PROJ_DIR}/saves/model_fusion/${MODEL_ID}/0525/Llama-3.2-3B-Instruct-sft-warmup"
...
for ((i = 1; i <= ITERATION_TIME; i++)); do
    ...
    if (( i == 1 )); then
        ...
        CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 FORCE_TORCHRUN=1 llamafactory-cli train \
            "$PROJ_DIR/recipes/align_dpo/iterative_reverse_segment_dpo_w_accu_q_hybrid/iter_${i}.yaml" > "$LOG_FILE" 2>&1
        ...
        ROLLOUT_INPUT_FILE="$PROJ_DIR/data/FuseChat-3.0-DPO-Data/$DATE/$MODEL_ID/$VERSION/rollout_forward/completion_with_rm_score/orm_verify/all_step_rollout_orm_verified.json"
        ...
        # python "$PROJ_DIR/rollout_for_math/build_dataset_0520.py" \
        #     --completion_file_dir "$ORM_SCORED_OUTPUT_DIR" \
        #     --post_process_type create_hybrid_policy_dpo_dataset \
        #     --iteration_id "$i" \
        #     --save_dir $PROJ_DIR/data/FuseChat-3.0-DPO-Data/$DATE/$MODEL_ID/$VERSION/iter_${i} > "$POST_PROCESS_LOG_FILE" 2>&1
