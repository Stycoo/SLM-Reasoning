...
VERSION='iterative_adv_based_misalign_dpo_forward'
...
do  
    ...
    SEEDS=(13 21 42 79 100) # 13 21 42 79
    ...
    while [ $JOB_INDEX -lt $NUM_SEEDS ]; do
        ...
        for (( CHUNK_ID=0; CHUNK_ID<CHUNK_NUM; CHUNK_ID++ )); do
            ...
            CUDA_VISIBLE_DEVICES=$GPU python $PROJ_DIR/completion_inference/completion_infer_in_specific_range_for_math_forward.py \
    ...
    POST_PROCESS_TYPE=create_step_adv_based_misaligned_dpo_dataset
    COMPLETION_DIR=$RM_SCORED_OUTPUT_DIR
    CREATE_DPO_LOG_FILE="${LOG_DIR}/create_adv_based_misaligned_dpo_iter_${i}.log"
    python $PROJ_DIR/completion_inference/build_dataset_v2.py \
    ...
    CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 FORCE_TORCHRUN=1 llamafactory-cli train \
        "$PROJ_DIR/recipes/dpo/iterative_adv_based_misalign_dpo_forward/iter_$i.yaml" > "$LOG_FILE" 2>&1
...
MODEL_NAMES=(iter_3) # iter_2 iter_3
DATATYPES=(aime)
N=(16)
