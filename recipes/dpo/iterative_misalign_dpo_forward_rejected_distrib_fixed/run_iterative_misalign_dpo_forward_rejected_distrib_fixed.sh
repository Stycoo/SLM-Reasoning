...
VERSION='iterative_misalign_dpo_forward_rejected_distrib_fixed'
...
do  
    ...
    if [ $i -eq 1 ]; then
        ...
        MODEL=$PROJ_DIR/saves/model_fusion/FuseChat-Llama-3.2-3B-SFT/0412/iterative_misalign_dpo_forward/iter_1
    else
    ...
    #         CUDA_VISIBLE_DEVICES=$GPU python $PROJ_DIR/completion_inference/completion_infer_in_specific_index_rejected_distrib_fixed.py \
    ...
    POST_PROCESS_TYPE=create_misaligned_dpo_forward_dataset
    ...
    CUDA_VISIBLE_DEVICES=0,1,2,3 FORCE_TORCHRUN=1 llamafactory-cli train \
        "$PROJ_DIR/recipes/dpo/iterative_misalign_dpo_forward_rejected_distrib_fixed/iter_$i.yaml" > "$LOG_FILE" 2>&1
...
MODEL_NAMES=(iter_2) # iter_2 iter_3
DATATYPES=(math)
N=(1)
