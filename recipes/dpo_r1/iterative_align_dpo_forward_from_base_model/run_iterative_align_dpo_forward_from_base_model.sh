...
VERSION='iterative_align_dpo_forward_from_base_model_v2'
...
do  
    ...
    while [ $JOB_INDEX -lt $NUM_SEEDS ]; do
        ...
        for (( CHUNK_ID=0; CHUNK_ID<CHUNK_NUM; CHUNK_ID++ )); do
            ...
            CUDA_VISIBLE_DEVICES=$GPU python $PROJ_DIR/completion_inference/completion_infer_in_specific_index_for_aligned_dpo_math.py \
    ...
    POST_PROCESS_TYPE=create_aligned_dpo_dataset
    ...
    CUDA_VISIBLE_DEVICES=0,1,2,3 FORCE_TORCHRUN=1 llamafactory-cli train \
        "$PROJ_DIR/recipes/dpo/iterative_align_dpo_forward_from_base_model/iter_$i.yaml" > "$LOG_FILE" 2>&1
